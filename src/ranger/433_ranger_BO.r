...
kBO_iter  <- 60   #cantidad de iteraciones de la Optimizacion Bayesiana
...
hs  <- makeParamSet(
          ...
          makeIntegerParam("max.depth",         lower=    1L, upper=   16L),  # 0 significa profundidad infinita
          makeIntegerParam("min.node.size" ,    lower=    1L, upper=  320L),
          makeIntegerParam("mtry" ,             lower=    2L, upper=   20L))


ksemilla_azar  <- 703447  #Aqui poner la propia semilla
...
setwd("C:\\Lab_Imp_1\\labo\\exp\\HT4330\\")   #Establezco el Working Directory DEL EXPERIMENTO
