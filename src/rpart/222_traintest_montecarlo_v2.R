...
ksemillas  <- c(703411,703441,703447,703471,703499)
...
# CARPETA DONDE VAN LOS RDOS?!
min.split = param_basicos$minsplit
min.bucket = param_basicos$minbucket
max.depth = param_basicos$maxdepth

for(minsplit in c(1,13,25,26,200))
{
for(minbucket in c(1,13))
{    
for(maxdepth in c(4,5,6))  
{
  # 
  # param_basicos<- list("cp"=-0.5
  #                      "minsplit"=min.split
  #                      ..... seguirrrrrrrr)
  # 19.16 hs clase 17052022
  ...
  cat( param_basicos$maxdepth,
       param_basicos$minsplit,
       param_basicos$minbucket,
       ganancia_final,
       "\n",
       file = ".exp/TH01/busqueda.txt",
       append = TRUE)
}
}
}
